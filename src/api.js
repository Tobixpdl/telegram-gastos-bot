...
function createApiRouter({ db, admin, ownerId, detectPlaceAndSubtype, getArgentinaDateParts, monthKey, calculateBillingMonth, getClosingDay, setClosingDayAndRecalculate, createExpense: persistExpense }) {
  ...
  async function personFields(personId) { if (!personId || personId === "self") { const ref=db.collection("expense_people").doc("self"); await ref.set({ownerId,name:"Yo",updatedAt:fieldValue.serverTimestamp()},{merge:true}); return {personId:"self",personName:"Yo"}; } const doc=await getOwned("expense_people",personId); return {personId:doc.id,personName:doc.data().name}; }
  async function createExpense(input) {
    ...
    const ref=await persistExpense({ chatId: ownerId, parsed, amountCents, originalText: input.description || `${input.place}${input.subtype ? ` ${input.subtype}` : ""} ${input.amount}`, expenseDate: date, source: "desktop", categoryId: input.categoryId }); await ref.update(await personFields(input.personId)); return ref;
  }
  router.get("/expenses/summary", async (req, res) => {
    const { billingMonth, personId } = req.query;
    ...
    snap.forEach((doc) => { const d=doc.data(); if(personId && !(personId === "self" ? (!d.personId || d.personId === "self") : d.personId === personId)) return; const cents=Number(d.amountCents||0); totalCents+=cents; for (const [map,key,label] of [[places,d.place||"sin categoria","place"],[subtypes,d.subtype||"general","subtype"]]) { const current=map.get(key)||{[label]:key,totalCents:0,count:0};current.totalCents+=cents;current.count+=1;map.set(key,current); } const categoryKey=d.categoryId||"__none__", category=categories.get(categoryKey)||{categoryId:d.categoryId||null,categoryName:d.categoryName||"Sin categoria",totalCents:0,count:0};category.totalCents+=cents;category.count+=1;categories.set(categoryKey,category); });
  ...
  router.get("/expenses", async (req, res) => {
    const { billingMonth, expenseMonth, place, subtype, dateFrom, dateTo, personId } = req.query;
    ...
    let query = db.collection("telegram_expenses").where("chatId", "==", ownerId);
    if (billingMonth) query = query.where("billingMonth", "==", billingMonth);
    if (expenseMonth) query = query.where("expenseMonth", "==", expenseMonth);
    const snap = await query.get();
    const rows = []; snap.forEach((doc) => { const d = documentJson(doc); if ((!billingMonth || d.billingMonth === billingMonth) && (!expenseMonth || d.expenseMonth === expenseMonth) && (!place || d.placeKey === String(place).toLowerCase()) && (!subtype || d.subtypeKey === String(subtype).toLowerCase()) && (!personId || (personId === "self" ? (!d.personId || d.personId === "self") : d.personId === personId)) && (!dateFrom || d.expenseDateIso >= dateFrom) && (!dateTo || d.expenseDateIso <= dateTo)) rows.push(expenseJson(d)); });
  ...
  router.post("/expenses/installments", async (req,res) => {
    assertObject(req.body); rejectUnknown(req.body,new Set(["place","subtype","total","installmentAmount","installmentCount","startBillingMonth","categoryId","personId"]));
  ...
  function personJson(row) { return {id:row.id,name:row.name,createdAt:timestampToIso(row.createdAt),updatedAt:timestampToIso(row.updatedAt)}; }
  router.get("/expense-people",async(req,res)=>{const self=db.collection("expense_people").doc("self");await self.set({ownerId,name:"Yo",updatedAt:fieldValue.serverTimestamp()},{merge:true});const s=await db.collection("expense_people").where("ownerId","==",ownerId).get();res.json({success:true,data:s.docs.map(d=>personJson(documentJson(d))).sort((a,b)=>a.id==="self"?-1:a.name.localeCompare(b.name))});});
  router.post("/expense-people",async(req,res)=>{assertObject(req.body);rejectUnknown(req.body,new Set(["name"]));const ref=await db.collection("expense_people").add({ownerId,name:requiredString(req.body.name,"name",80),createdAt:fieldValue.serverTimestamp(),updatedAt:fieldValue.serverTimestamp()});res.status(201).json({success:true,data:personJson(documentJson(await ref.get()))});});
  router.patch("/expense-people/:id",async(req,res)=>{if(req.params.id==="self")throw new ApiError(400,"VALIDATION_ERROR","La persona Yo no se puede renombrar.");assertObject(req.body);rejectUnknown(req.body,new Set(["name"]));const d=await getOwned("expense_people",req.params.id);await d.ref.update({name:requiredString(req.body.name,"name",80),updatedAt:fieldValue.serverTimestamp()});res.json({success:true,data:personJson(documentJson(await d.ref.get()))});});  router.delete("/expense-people/:id",async(req,res)=>{if(req.params.id==="self")throw new ApiError(400,"VALIDATION_ERROR","La persona Yo no se puede eliminar.");const d=await getOwned("expense_people",req.params.id);const expenses=await db.collection("telegram_expenses").where("chatId","==",ownerId).where("personId","==",d.id).get();if(!expenses.empty)throw new ApiError(409,"PERSON_IN_USE","No podés eliminar una persona con gastos asociados.");await d.ref.delete();res.status(204).end();});
  ...
  router.get("/expenses/analytics",async(req,res)=>{const {fromMonth,toMonth,groupBy="category",monthField="billingMonth"}=req.query;if(!MONTH_RE.test(fromMonth||"")||!MONTH_RE.test(toMonth||""))throw new ApiError(400,"VALIDATION_ERROR","Los meses deben tener el formato YYYY-MM.");if(!["category","person"].includes(groupBy))throw new ApiError(400,"VALIDATION_ERROR","groupBy debe ser category o person.");if(!["billingMonth","expenseMonth"].includes(monthField))throw new ApiError(400,"VALIDATION_ERROR","monthField debe ser billingMonth o expenseMonth.");const s=await db.collection("telegram_expenses").where("chatId","==",ownerId).where(monthField,">=",fromMonth).where(monthField,"<=",toMonth).get();const rows={};const groups=new Map();s.forEach(d=>{const x=d.data(),month=x[monthField];if(!month)return;const id=groupBy==="category"?(x.categoryId||"__uncategorized__"):(x.personId||"self"),name=groupBy==="category"?(x.categoryName||"Sin categoría"):(x.personName||"Yo"),color=groupBy==="category"?(id==="__uncategorized__"?"#7d8797":null):(id==="self"?"#4f6bed":null),r=rows[month]||(rows[month]={month,totalCents:0,groups:{}}),cents=Number(x.amountCents||0);r.totalCents+=cents;r.groups[id]=(r.groups[id]||0)+cents;if(!groups.has(id))groups.set(id,{id,name,color});});res.json({success:true,data:{months:Object.values(rows).sort((a,b)=>a.month.localeCompare(b.month)),groups:[...groups.values()]}});});  return router;
}
